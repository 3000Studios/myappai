...
.DESCRIPTION
  - Applies an existing patch (0001-fix-stabilize-build.patch) or can be used after manual changes.
  - Runs pnpm install, prebuild, type-check and build to validate.
  - Runs the seed-admin.ts script to create admin user in the local store (reads ADMIN_EMAIL/ADMIN_PASSWORD env vars).
  - Pushes the branch to origin, creates a PR using gh, and (optionally) merges the PR.
  - Optionally sets repository secrets interactively via gh (ADMIN_EMAIL, ADMIN_PASSWORD, DATABASE_URL, NEXTAUTH_SECRET, VERCEL_TOKEN).
  - Polls the public site URL for a successful deployment after merge.

USAGE
  - Ensure prerequisites: git, pnpm, node, gh (GitHub CLI), (optional) vercel
  - Place your patch at ./0001-fix-stabilize-build.patch or run after applying local code edits.
  - Run with: powershell -ExecutionPolicy Bypass -File .\scripts\create-pr-and-deploy.ps1

PARAMETERS
  -PatchFile     Path to patch file (default ./0001-fix-stabilize-build.patch)
  -Branch        Branch name to create (default fix/stabilize-build-0001)
  -PushToMain    If set, attempt to merge branch into main and push (only allowed if you have rights)
  -AutoMergePR   If set, attempt to merge the PR via gh (requires permissions and passing CI)
#>
...
# optional: run prebuild if present
...
Run the included scripts locally to seed an admin account and build.
"@
...
Log "If you want me to attempt deployment via Vercel CLI, run: npm i -g vercel; vercel --prod --token <YOUR_TOKEN>"
exit 0
